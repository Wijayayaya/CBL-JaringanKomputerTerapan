...
async function loadRecords() {
  ...
  try {
    const [recRes, patRes] = await Promise.allSettled([
      fetch(`${SERVICE_B}/medical-records`),
      fetch(`${SERVICE_A}/patients`)
    ]);
    ...
    countEl.textContent = serviceBOnline
      ? `${displayRecords.length} rekam medis`
      : `${cachedRecords ? cachedRecords.length : '?'} rekam medis (cache)`;

    // Comparison bar
    const diff = (totalPatients !== null && records !== null) ? totalPatients - displayRecords.length : null;
    const comparisonHtml = (totalPatients !== null && records !== null) ? `
    ...
    if (displayRecords.length === 0) {
      container.innerHTML = statusBanner + comparisonHtml + `<div class="list-table-wrap" style="padding:40px;text-align:center">
        <div style="font-size:2rem;margin-bottom:8px">🗂️</div>
    ...
    container.innerHTML = statusBanner + comparisonHtml + `
      <div class="list-table-wrap">
...
async function fetchRecord(patientName) {
  ...
  if (!pid) { showToast('Masukkan Patient ID dulu', 'error'); return; }
  ...
  try {
    ...
    const statusClass = { draft: 'status-draft', active: 'status-active', archived: 'status-archived' }[data.status] || 'status-draft';
    const statusLabel = { draft: '⏳ Draft', active: '✅ Aktif', archived: '📁 Arsip' }[data.status] || data.status;

    const allergiesHtml = data.allergies && data.allergies.length > 0
      ? data.allergies.map(a => `<span class="allergy-tag ${a.is_critical ? 'critical' : 'normal'}">
          ${a.is_critical ? '⚠️ ' : ''}${escHtml(a.label)} <small>(${escHtml(a.code)})</small>
        </span>`).join('')
      : `<span class="empty-state">Tidak ada data alergi</span>`;

    const visitsHtml = data.visit_history && data.visit_history.length > 0
      ? `<table class="visit-table">
          <thead><tr><th>Tanggal</th><th>Klinik</th><th>Diagnosis</th><th>Catatan Dokter</th><th>Dibuat</th></tr></thead>
          <tbody>
            ${data.visit_history.map(v => `<tr>
    ...
    container.innerHTML = `
      <div class="record-hero">
        ...
        <div class="record-hero-status ${statusClass}">${statusLabel}</div>
